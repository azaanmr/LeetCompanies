...
export default function GlobalSearchTab() {
  ...
  const [subTab, setSubTab] = useState('search'); // 'search' | 'overlap'
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'center', marginBottom: '2rem' }}>
        <div className="nav-tabs" style={{ padding: '0.35rem' }}>
          <button
            className={`nav-tab-btn ${subTab === 'search' ? 'active' : ''}`}
            onClick={() => setSubTab('search')}
            style={{ padding: '0.6rem 1.25rem' }}
          >
            <Search size={16} />
            <span>Universal Problem Search</span>
          </button>

          <button
            className={`nav-tab-btn ${subTab === 'overlap' ? 'active' : ''}`}
            onClick={() => setSubTab('overlap')}
            style={{ padding: '0.6rem 1.25rem' }}
          >
            <Layers size={16} />
            <span>Multi-Company Overlap Matrix</span>
          </button>
        </div>
      </div>
          ...
          <section className="hero-header">
            <div className="hero-main">
              ...
              <p className="hero-subtitle">
                Search any LeetCode problem to see every company that asks it, along with question frequencies.
              </p>
            </div>
          </section>
                    ...
                    <div style={{ marginTop: '0.25rem' }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.4rem' }}>
                        {prob.companies.slice(0, 12).map((comp) => (
                          ...
                          >
                            <CompanyLogo slug={comp.slug} name={comp.name} size={18} fontSize="0.65rem" />
